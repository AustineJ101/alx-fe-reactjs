...
function Search () {
    ...
    const handleSubmit = async (e) => {
        ...
        if(data){
            setUserData({name: data.name, login: data.login, avatar: data.avatar_url, link: data.html_url});
            setIsLoading(false);
        }else{
        ...
        <div className="m-4">
            ...
            {userData && (
                <div className="flex items-center gap-4 ">
                    ...
                    <a className="underline text-blue-500" href={userData.link} target="_blank">{userData.login}</a>
                </div>
            )}
        </div>
